...
public class BaseViewOffsetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {

    private BaseViewOffsetHelper mViewOffsetHelper;

    private int mTempTopBottomOffset = 0;
    private int mTempLeftRightOffset = 0;

    public BaseViewOffsetBehavior() {}

    public BaseViewOffsetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mViewOffsetHelper == null) {
            mViewOffsetHelper = new BaseViewOffsetHelper(child);
        }
